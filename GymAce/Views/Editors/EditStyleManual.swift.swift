...
struct EditManual: View {
    ...
    var body: some View {
        VStack {
            ...
            Spacer()
            Text(Style.manual(findInfo()).description())
                .padding(.leading, 10)
        }
        .navigationTitle("Edit Manual")
    ...
    private var warmupBinding: Binding<String> {
        Binding(
            ...
            set: {
                ...
                var info = findInfo()
                info.warmup = a
                program.styles[name] = .manual(info)
                warmupErr = nil
            }
        )
    }
    ...
    private func findInfo() -> VariableInfo {
        ...
        if case .manual(let info) = style {
            return info
        }
        fatalError("Expected \(name) to be manual style")
    }
...
#Preview {
    ...
    NavigationView {
        EditManual(model: model, program: program, name: "Manual")
    }
}
